...
version: 1.0.0
*/
import { Vector3 as Vector3$1, Face3 as Face3$1, BufferGeometry, Vector2, BufferAttribute, Geometry, Matrix4, Mesh, Vector4 } from 'three';
...
function () {
  function ThreeGeometry(geometry) {
    ...
    if (this._isBufferGeometry) {
      this._processingGeometry = new Geometry().fromBufferGeometry(geometry);
    } else {
  ...
  __proto.update = function (datas) {
    ...
    geometry.vertices = vertices.map(function (vertex) {
      return new Vector3$1(vertex.x, vertex.y, vertex.z);
    });
    geometry.faces = faces.map(function (face) {
      return new Face3$1(face.a, face.b, face.c);
    });
    ...
    if (this._isBufferGeometry) {
      ...
      var bufferGeo = new BufferGeometry().fromGeometry(geometry);
      ...
      if (hasTangent && hasUV) {
        ...
        faces.forEach(function (face, faceIdx) {
          ...
          var dPos0 = new Vector3$1().subVectors(faceVertices[1], faceVertices[0]);
          var dPos1 = new Vector3$1().subVectors(faceVertices[2], faceVertices[0]);
          var dUV0 = new Vector2().subVectors(faceUVs[1], faceUVs[0]);
          var dUV1 = new Vector2().subVectors(faceUVs[2], faceUVs[0]);
        ...
        bufferGeo.setAttribute("tangent", new BufferAttribute(tangents_1, 4));
      }
  ...
  __proto._skinnedMeshToMesh = function (skinnedMesh, skeleton) {
    ...
    var _loop_1 = function (posIdx) {
      ...
      weights.forEach(function (weight, index) {
        var boneMatrix = new Matrix4().fromArray(boneMatricies, indicies[index] * 16).multiplyScalar(weight);
    ...
    var mesh = new Mesh(skinnedMesh.geometry, skinnedMesh.material).copy(skinnedMesh);
